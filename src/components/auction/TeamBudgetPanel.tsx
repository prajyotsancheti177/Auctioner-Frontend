...
export const TeamBudgetPanel = ({
  ...
  return (
    <>
      ...
      <div className="hidden md:flex flex-col w-56 lg:w-64 shrink-0 bg-card/80 backdrop-blur-sm border-2 border-border rounded-2xl shadow-elevated overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1.5 custom-scrollbar">
          {teams.map((team) => {
            const noSlots =
              (team.maxPlayersPerTeam ?? 0) - (team.playersCount ?? 0) <= 0;
            const insufficientBudget =
              (team.remainingBudget ?? 0) < nextBidAmount;
            const exceedsMaxBiddable =
              (team.maxBiddableAmount ?? 0) < nextBidAmount;
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  >
                    Max: {team.maxBiddableAmount ?? 0} Pts
                  </p>
                </div>
                <div className="text-right flex-shrink-0">
                  <p className="text-[10px] text-muted-foreground leading-tight">
                    {(team.maxPlayersPerTeam || 0) - (team.playersCount || 0)} slots
                  </p>
                </div>
              </div>
            );
